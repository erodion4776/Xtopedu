...
// ✅ Fixed: searchStudents handles full name search
// ✅ Fixed: Split words for first+last name combo
// ✅ Fixed: Reversed name order search
// ============================================================
...
export class AdminService {
    ...
    for (const variant of variants) {
      const { data } = await db
        ...
        .select(`
          ...
          schools ( id, name, is_active, subscription_status )
        `)
        .or(
          `phone.eq.${variant},` +
          `whatsapp_number.eq.${variant}`
        )
      ...
      if (data?.school_users) {
        const school =
          (data as Record<string, unknown>).schools as
            | { is_active: boolean }
            | null;
  ...
  isAdmin(schoolUser: SchoolUser): boolean {
    const roleName =
      schoolUser.roles?.name?.toLowerCase() ?? '';
    return [
  ...
  async getClassStudents(
    ...
    let query = db
      ...
      .select(`
        ...
        admission_number, status, gender,
        class_id, class_arm_id,
        classes ( id, name ),
        class_arms ( id, name )
      `)
      .eq('class_id', classId)
  ...
  // ✅ Fixed: Handles full name search (John Doe)
  // ✅ Fixed: Handles reversed name (Doe John)
  // ✅ Fixed: Searches first name, last name independently
  async searchStudents(
    ...
    // Split into words so "John Doe" → ['John', 'Doe']
    const words  = query
      .split(/\s+/)
      .filter((w) => w.length > 0);
    const first  = words[0] ?? '';
    ...
    if (second) {
      // ✅ Two words typed — search both combos:
      // "John Doe" → first=John, last=Doe
      // "Doe John" → first=Doe, last=John
      // Also check admission number
      dbQuery = dbQuery.or(
        `and(first_name.ilike.%${first}%,` +
        `last_name.ilike.%${second}%),` +
        `and(first_name.ilike.%${second}%,` +
        `last_name.ilike.%${first}%),` +
        `admission_number.ilike.%${query}%`
      );
    } else {
      // ✅ One word — search first name, last name,
      // or admission number
      dbQuery = dbQuery.or(
        `first_name.ilike.%${first}%,` +
        `last_name.ilike.%${first}%,` +
        `admission_number.ilike.%${first}%`
      );
    }
  ...
  async getStudentOutstandingFees(
    ...
    const invoices = (
      ...
      return {
        ...
        balance:
          parseFloat(String(inv.balance ?? 0)),
        amount:
          parseFloat(String(inv.amount ?? 0)),
        amount_paid:
          parseFloat(String(inv.amount_paid ?? 0)),
    ...
    const total = invoices.reduce(
      (s, inv) => s + (inv.balance as number), 0
    );
  ...
  async getStaff(
    ...
    const { data } = await db
      ...
      .select(`
        ...
        whatsapp_number, department,
        employment_status,
        staff_invitations ( status, token, expires_at )
      `)
      .eq('school_id', schoolId)
      .eq('employment_status', 'active')
      .order('first_name', { ascending: true });
  ...
  async getAttendanceSettings(
    ...
    const { data } = await db
      ...
      .select(
        'notify_absent, notify_late, ' +
        'notify_present, auto_close_hour'
      )
      .eq('school_id', schoolId)
      .single();
  ...
  async getBroadcastTargets(
    ...
    if (target === 'class_parents' && classId) {
      ...
      for (const sp of data ?? []) {
        const p =
          sp.parents as Record<string, string> | null;
        if (!p) continue;
    ...
    if (target === 'debtors') {
      ...
      for (const inv of data ?? []) {
        const student =
          inv.students as Record<string, unknown> | null;
        const sps =
          student?.student_parents as
            Record<string, unknown>[] | null;
  ...
  async notifyParentOfPayment(
    ...
    try {
      ...
      const parent  =
        sp.parents as Record<string, string> | null;
      const student =
        sp.students as Record<string, string> | null;
      ...
      console.warn(
        '[AdminService] notifyParent error:', err
      );
    }
  }
  ...
  // ─── Map DB row to Student type ────────────────────────────────────────
  private mapStudent(
    s: Record<string, unknown>
  ): Student {
    const cls =
      s.classes as { id: string; name: string } | null;
    const arm =
      s.class_arms as { id: string; name: string } | null;
