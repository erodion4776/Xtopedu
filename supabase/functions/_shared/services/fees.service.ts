...
export class FeesService {
  ...
  async getOutstanding(
    ...
    if (error) {
      console.error('[FeesService] getOutstanding error:', error);
      return { invoices: [], total: 0 };
    }

    const invoices = ((data ?? []) as Record<string, unknown>[]).map(
      (row) => {
        const fs = row.fee_structures as Record<string, unknown> | null;
        ...
        return {
          ...
          amount_paid: parseFloat(String(row.amount_paid ?? 0)),
          balance: parseFloat(String(row.balance ?? 0)),
          status: row.status as string,
          due_date: row.due_date as string | null,
          // Use fee structure title or fallback to invoice number
          title:
            (fs?.title as string) ??
  ...
  async getHistory(
    ...
    if (error) {
      console.error('[FeesService] getHistory error:', error);
      return [];
    }

    return ((data ?? []) as Record<string, unknown>[]).map((p) => {
      const inv = p.student_invoices as Record<string, unknown> | null;
  ...
  // Shows total billed, paid and outstanding
  async getSummary(
    studentId: string,
    schoolId: string
  ): Promise<{
    ...
    return {
      totalBilled: rows.reduce((s, r) => s + n(r.amount), 0),
      totalPaid: rows.reduce((s, r) => s + n(r.amount_paid), 0),
      totalOutstanding: rows.reduce((s, r) => s + n(r.balance), 0),
  ...
  // Returns payment URL for parent to tap and pay
  async startPayment(params: {
    ...
    try {
      ...
      // So we have a record even if webhook is slow
      const { error: insertError } = await db
        ...
        .insert({
          ...
          // Save school fee amount (not total parent pays)
      ...
      const result = await paystack.initializeSchoolFeePayment({
        email: params.email || `${params.phone}@schoolbot.ng`,
        ...
        callbackUrl:
          `${appUrl}/functions/v1/payment-callback?ref=${reference}`,
      });
      ...
      console.error('[FeesService] startPayment error:', err);
      return null;
    }
  }
  ...
  async getInvoicesDueSoon(
    ...
    const dueBefore = futureDate.toISOString().split('T')[0];
  ...
  // Used by fee reminder cron job
  async getOverdueInvoices(): Promise<Record<string, unknown>[]> {
    const today = new Date().toISOString().split('T')[0];
